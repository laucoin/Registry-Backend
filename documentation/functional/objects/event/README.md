...
* **Name**\
  The name is the human identifier for the Event. It must not be unique but it's recommended to create the most explicit
  name.
* **Start date and time**\
...
* **Options**\
  Some features can be enabled for specifics events. The list of available option should be defined in the organization
  config.
* **Scheduled display threshold**\
  This is the threshold at which scheduled movements are shown.
* **Hidden**\
  An Event can be hide and restore. Obviously, nothing related to a disabled event can be perform except edit, re-enable
  and delete it.
