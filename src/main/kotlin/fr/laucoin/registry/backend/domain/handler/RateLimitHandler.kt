...
class RateLimitHandler(
	...
	private val counters: Cache<String, FixedWindowCounter> = Caffeine.newBuilder()
		...
		return exchange.getPrincipal<Principal>()
			...
			.flatMap { principal ->
				val allowed = counters.get("$principal|$category") {
					FixedWindowCounter(capacity, Duration.ofSeconds(window))
				}.tryConsume()
	...
	/**
	 * A fixed window counter: the window resets to a fresh count once [window] has elapsed since it
	 * started, rather than continuously sliding — a caller can burst up to `2 * capacity` requests
	 * across a window boundary. Good enough for a coarse per-endpoint-category limiter; swap for a
	 * true sliding-window/token-bucket if that burst becomes a real concern.
	 */
	private class FixedWindowCounter(private val capacity: Int, private val window: Duration) {
