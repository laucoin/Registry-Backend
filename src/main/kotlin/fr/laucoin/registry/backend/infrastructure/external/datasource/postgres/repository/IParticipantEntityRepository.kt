...
import fr.laucoin.registry.backend.infrastructure.external.datasource.postgres.entity.participant.ParticipantQueries.PARTICIPANT_MAJOR_CLAUSE
...
interface IParticipantEntityRepository: ReactiveCrudRepository<ParticipantEntity, UUID> {
    ...
    @Query(
        ...
        FROM $PARTICIPANT_TABLE t $USER_JOIN $LAST_MOVEMENT_JOIN $GROUPS_JOIN $PROJECT_JOIN $CREATOR_JOIN $LAST_EDITOR_JOIN
        WHERE $NOT_PURGED_CLAUSE AND $PROJECT_CLAUSE AND (t.$PARTICIPANT_BIRTHDAY = CURRENT_DATE) IN (:ids) AND $VISIBLE_CLAUSE
        """
    )
