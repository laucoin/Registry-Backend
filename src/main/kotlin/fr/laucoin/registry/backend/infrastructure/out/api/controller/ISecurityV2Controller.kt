...
/**
 * API v2 Authentication contract (ADR 017). Same shape as v1: the four
 * provider endpoints are public (SecurityConfig allow-list), the current-user
...
interface ISecurityV2Controller {
	...
	fun getLoginUri(@RequestParam @Valid @NotBlank(message = REDIRECT_URI_BLANK) redirectUri: String?): AuthenticationUriReaderDto
	...
	fun getLogoutUri(@RequestParam @Valid @NotBlank(message = REDIRECT_URI_BLANK) redirectUri: String?): AuthenticationUriReaderDto
	...
	@PostMapping("/token")
	fun fetchToken(@RequestBody @Valid authenticationInfo: AuthenticationInfoWriterDto): Mono<TokenReaderDto>
	...
	fun refreshToken(@RequestBody @Valid refreshAuthenticationInfo: RefreshAuthenticationInfoWriterDto): Mono<TokenReaderDto>
