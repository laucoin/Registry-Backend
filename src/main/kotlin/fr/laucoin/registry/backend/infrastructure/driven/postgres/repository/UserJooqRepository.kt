...
class UserJooqRepository(private val dsl: DSLContext) {
	private fun notPurgedAndNotServiceAccount(): Condition = TB_USER.PURGED.isFalse.and(TB_USER.TYPE.ne(UserTypeEnum.SERVICE_ACCOUNT))
	...
	fun findAll(textSearched: String?, visibilitySearched: Boolean?, limit: Int, offset: Int): Flux<UserEntity> {
		...
		val similarityScore = (if (textSearched == null) DSL.inline(1f) else similarity(TB_USER.SEARCH_TEXT, DSL.`val`(textSearched))).`as`("similarity_score")
		val fullCount = count().over().`as`("full_count")
		return Flux.from(
			dsl.select(TB_USER.asterisk(), similarityScore, creator.FIRST_NAME, creator.LAST_NAME, creator.EMAIL, editor.FIRST_NAME, editor.LAST_NAME, editor.EMAIL, fullCount)
				.from(TB_USER)
	...
	fun findWithLimit(textSearched: String?, visibilitySearched: Boolean?, limit: Int): Flux<UserEntity> {
		...
		val similarityScore = (if (textSearched == null) DSL.inline(1f) else similarity(TB_USER.SEARCH_TEXT, DSL.`val`(textSearched))).`as`("similarity_score")
		return Flux.from(
			dsl.select(TB_USER.asterisk(), similarityScore, creator.FIRST_NAME, creator.LAST_NAME, creator.EMAIL, editor.FIRST_NAME, editor.LAST_NAME, editor.EMAIL)
				.from(TB_USER)
				.leftJoin(creator).on(TB_USER.CREATED_BY.eq(creator.ID))
				.leftJoin(editor).on(TB_USER.LAST_MODIFIED_BY.eq(editor.ID))
				.where(notPurgedAndNotServiceAccount().and(searchCondition(textSearched)).and(visibleCondition(TB_USER.VISIBLE, visibilitySearched)))
	...
	fun findById(id: UUID, visibilitySearched: Boolean?): Mono<UserEntity> {
		...
		val editor = editorTable()
		return Mono.from(
			dsl.select(TB_USER.asterisk(), creator.FIRST_NAME, creator.LAST_NAME, creator.EMAIL, editor.FIRST_NAME, editor.LAST_NAME, editor.EMAIL)
				.from(TB_USER)
				.leftJoin(creator).on(TB_USER.CREATED_BY.eq(creator.ID))
				.leftJoin(editor).on(TB_USER.LAST_MODIFIED_BY.eq(editor.ID))
				.where(notPurgedAndNotServiceAccount().and(TB_USER.ID.eq(id)).and(visibleCondition(TB_USER.VISIBLE, visibilitySearched)))
		).map { it.toEntity(creator, editor) }
	}

	// No PREFERENCES_JOIN, matching the original: preference fields on the returned CurrentUserEntity
	// are always null here — a pre-existing quirk of the query this method has always had, not a bug
	...
	fun findByOidcId(oidcId: UUID, visibilitySearched: Boolean?): Mono<CurrentUserEntity> {
		...
		return Mono.from(
			preferencesSelect(creator, editor)
				.where(TB_USER.TYPE.ne(UserTypeEnum.SERVICE_ACCOUNT).and(TB_USER.OIDC_ID.eq(oidcId)).and(visibleCondition(TB_USER.VISIBLE, visibilitySearched)))
	...
	fun findByEmail(email: String, visibilitySearched: Boolean?): Flux<CurrentUserEntity> {
		...
		return Flux.from(
			preferencesSelect(creator, editor)
				.where(TB_USER.TYPE.ne(UserTypeEnum.SERVICE_ACCOUNT).and(TB_USER.EMAIL.eq(email)).and(visibleCondition(TB_USER.VISIBLE, visibilitySearched)))
		).map { it.toCurrentUserEntity(creator, editor, includesPreferences = true) }
	}
	...
	private fun preferencesSelect(creator: TbUser, editor: TbUser) =
		dsl.select(
			TB_USER.asterisk(),
			TB_PREFERENCES.ID, TB_PREFERENCES.THEME, TB_PREFERENCES.LANGUAGE, TB_PREFERENCES.SELECTED_PROFILE_ID,
			TB_PROJECT.ID, TB_PROJECT.NAME, TB_PROJECT.BEGIN_DATE, TB_PROJECT.BEGIN_TIME, TB_PROJECT.END_DATE, TB_PROJECT.END_TIME, TB_PROJECT.OPTIONS,
			TB_PROJECT_PROFILE.ROLE, TB_PROJECT_PROFILE.STATUS, TB_PROJECT_PROFILE.START_ACCESS_DATE, TB_PROJECT_PROFILE.START_ACCESS_TIME, TB_PROJECT_PROFILE.END_ACCESS_DATE, TB_PROJECT_PROFILE.END_ACCESS_TIME,
			...
			.leftJoin(TB_PROJECT_PROFILE).on(TB_PREFERENCES.SELECTED_PROFILE_ID.eq(TB_PROJECT_PROFILE.ID).and(TB_PROJECT_PROFILE.VISIBLE.isTrue))
	...
	fun findUserIdsOlderThanLastLogin(dateThreshold: LocalDate): Flux<UUID> = Flux.from(
		dsl.select(TB_USER.ID)
			.from(TB_USER)
			.where(DSL.condition("{0}::date < {1}", TB_USER.LAST_LOGIN, DSL.`val`(dateThreshold)).and(TB_USER.TYPE.ne(UserTypeEnum.SERVICE_ACCOUNT)))
	).map { it.value1()!! }
	...
	private fun insert(entity: UserEntity): Mono<UserEntity> {
		...
		entity.visible?.let { record.set(TB_USER.VISIBLE, it) }
		entity.createdAt?.let { record.set(TB_USER.CREATED_DATE, it) }
		entity.creatorId?.let { record.set(TB_USER.CREATED_BY, it) }
		entity.lastUpdateAt?.let { record.set(TB_USER.LAST_MODIFIED_DATE, it) }
		entity.lastEditorId?.let { record.set(TB_USER.LAST_MODIFIED_BY, it) }
	...
	private fun update(entity: UserEntity): Mono<UserEntity> = Mono.from(
		dsl.update(TB_USER)
			.set(TB_USER.VISIBLE, entity.visible)
			.set(TB_USER.CREATED_DATE, entity.createdAt)
			.set(TB_USER.CREATED_BY, entity.creatorId)
			.set(TB_USER.LAST_MODIFIED_DATE, entity.lastUpdateAt)
			.set(TB_USER.LAST_MODIFIED_BY, entity.lastEditorId)
	...
	private fun Record.fillUserFields(entity: UserEntity, creator: TbUser?, editor: TbUser?) {
		entity.id = get(TB_USER.ID)
		entity.visible = get(TB_USER.VISIBLE)
		entity.createdAt = get(TB_USER.CREATED_DATE)
		entity.creatorId = get(TB_USER.CREATED_BY)
		...
		entity.lastUpdateAt = get(TB_USER.LAST_MODIFIED_DATE)
		entity.lastEditorId = get(TB_USER.LAST_MODIFIED_BY)
	...
	private fun Record.toEntity(creator: TbUser? = null, editor: TbUser? = null, fullCount: Field<Int>? = null): UserEntity =
		UserEntity(
			...
			fillUserFields(this, creator, editor)
			this.fullCount = fullCount?.let { get(it)?.toLong() }
		}

	// includesPreferences is passed explicitly by each caller (not detected via record.field(...) at
	// runtime): jOOQ's Record.field(Field) falls back to matching by bare column name when the exact
	// Field reference isn't in the row, so a probe like TB_PREFERENCES.ID silently matches TB_USER.ID
	// instead (both are just named "id") on a query that never joined preferences at all.
	private fun Record.toCurrentUserEntity(creator: TbUser? = null, editor: TbUser? = null, includesPreferences: Boolean): CurrentUserEntity {
