...
class KeycloakAuthenticationAdapter(
    ...
    private fun fetchToken(body: FormInserter<String>): Mono<TokenModel> {
        return http.post().uri("$baseUri$TOKEN_PATH")
            ...
            .onStatus(
                { it.is4xxClientError },
                { Mono.error(RegistryExceptionModel(HttpStatus.valueOf(it.statusCode().value()), AUTH_PROVIDER_4XX_ERROR)) })
