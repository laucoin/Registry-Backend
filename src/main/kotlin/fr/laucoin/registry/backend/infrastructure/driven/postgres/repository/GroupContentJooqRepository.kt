...
class GroupContentJooqRepository(private val dsl: DSLContext) {
	fun findAllByGroupIds(projectId: UUID, groupIds: List<UUID>, visibilitySearched: Boolean?, availabilitySearched: Boolean?): Flux<GroupContentEntity> {
		...
		availabilitySearched?.let {
			val started = TB_PARTICIPANT.START_AVAILABILITY_DATE.isNull
				.or(TB_PARTICIPANT.START_AVAILABILITY_DATE.lt(DSL.currentLocalDate()))
				.or(TB_PARTICIPANT.START_AVAILABILITY_DATE.eq(DSL.currentLocalDate()).and(TB_PARTICIPANT.START_AVAILABILITY_TIME.isNull.or(TB_PARTICIPANT.START_AVAILABILITY_TIME.le(DSL.currentOffsetTime()))))
			val notEnded = TB_PARTICIPANT.END_AVAILABILITY_DATE.isNull
				.or(TB_PARTICIPANT.END_AVAILABILITY_DATE.gt(DSL.currentLocalDate()))
				.or(TB_PARTICIPANT.END_AVAILABILITY_DATE.eq(DSL.currentLocalDate()).and(TB_PARTICIPANT.END_AVAILABILITY_TIME.isNull.or(TB_PARTICIPANT.END_AVAILABILITY_TIME.ge(DSL.currentOffsetTime()))))
			val isAvailable = started.and(notEnded)
			conditions += if (it) isAvailable else isAvailable.not()
		}
		return Flux.from(
			dsl.select(TB_GROUP_CONTENT.asterisk(), TB_PARTICIPANT.FIRST_NAME, TB_PARTICIPANT.LAST_NAME, TB_PARTICIPANT.BIRTHDAY, TB_PARTICIPANT.TYPE)
				.from(TB_GROUP_CONTENT)
				.join(TB_GROUP).on(TB_GROUP_CONTENT.GROUP_ID.eq(TB_GROUP.ID))
				.join(TB_PARTICIPANT).on(TB_GROUP_CONTENT.PARTICIPANT_ID.eq(TB_PARTICIPANT.ID).and(TB_PARTICIPANT.PURGED.isFalse))
	...
	fun save(entity: GroupContentEntity, using: DSLContext): Mono<GroupContentEntity> {
		...
		return Mono.from(using.insertInto(TB_GROUP_CONTENT).set(record).returning()).map { it.toEntity(includeParticipantInfo = false) }
	}
	...
	fun deleteAllByGroupIdAndParticipantIds(groupId: UUID, participantIds: List<UUID>, using: DSLContext): Mono<Unit> =
		Mono.from(using.deleteFrom(TB_GROUP_CONTENT).where(TB_GROUP_CONTENT.GROUP_ID.eq(groupId).and(TB_GROUP_CONTENT.PARTICIPANT_ID.`in`(participantIds)))).map { }
	...
	fun deleteAllByParticipantIdAndGroupIds(participantId: UUID, groupIds: List<UUID>, using: DSLContext): Mono<Unit> =
		Mono.from(using.deleteFrom(TB_GROUP_CONTENT).where(TB_GROUP_CONTENT.PARTICIPANT_ID.eq(participantId).and(TB_GROUP_CONTENT.GROUP_ID.`in`(groupIds)))).map { }
