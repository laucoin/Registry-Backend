...
class ProjectJooqRepository(private val dsl: DSLContext) {
	private fun searchConditions(textSearched: String?, visibilitySearched: Boolean?, dateTimeSearched: ZonedDateTime?): Condition {
	...
	fun findAll(textSearched: String?, visibilitySearched: Boolean?, dateTimeSearched: ZonedDateTime?, limit: Int, offset: Int): Flux<ProjectEntity> {
		val creator = creatorTable()
		val editor = editorTable()
		val fullCount = count().over().`as`("full_count")
		return Flux.from(
			dsl.select(TB_PROJECT.asterisk(), creator.FIRST_NAME, creator.LAST_NAME, creator.EMAIL, editor.FIRST_NAME, editor.LAST_NAME, editor.EMAIL, fullCount)
				.from(TB_PROJECT)
	...
	fun findById(id: UUID, visibilitySearched: Boolean?): Mono<ProjectEntity> {
		...
		val editor = editorTable()
		return Mono.from(
			dsl.select(TB_PROJECT.asterisk(), creator.FIRST_NAME, creator.LAST_NAME, creator.EMAIL, editor.FIRST_NAME, editor.LAST_NAME, editor.EMAIL)
				.from(TB_PROJECT)
				.leftJoin(creator).on(TB_PROJECT.CREATED_BY.eq(creator.ID))
				.leftJoin(editor).on(TB_PROJECT.LAST_MODIFIED_BY.eq(editor.ID))
	...
	// Mirrors IProjectEntityRepository.validDateTime: counts entities across every project-scoped
	// table whose availability/occurrence window does NOT overlap [begin, end] — a non-zero count
	// means shrinking the project's dates would orphan something.
	// Binds the full begin/end instants once each and lets Postgres extract the date/time parts
	// server-side (CAST(... AS date/time)) — extracting them in Kotlin first and binding separate
	// DATE/TIME parameters instead would round-trip an out-of-range TIMETZ offset for extreme
	// ZonedDateTime values (e.g. OffsetDateTime.MIN/MAX, ±18:00), which Postgres rejects outright.
	// Postgres timestamp/date can represent roughly 4713 BC .. 294276 AD; java.time's own extremes
	// (OffsetDateTime.MIN/MAX, used by tests to mean "unbounded past/future") go far beyond that AND
	// carry a +/-18:00 offset outside Postgres's +/-15:59:59 limit, so binding them directly fails.
	// Postgres's own 'infinity'/'-infinity' pseudo-values exist for exactly this "unbounded" case and
	// compare correctly against every finite value, so clamp to those instead of the raw input.
	...
	// Only reached when the date side has already clamped to +/-infinity, making the "date equals"
	// branch that guards this time comparison always false — so any safe (non-extreme-offset) value works.
	...
	private fun dateTimeNonOverlapCondition(dateTime: Field<ZonedDateTime?>, begin: ZonedDateTime?, end: ZonedDateTime?): Condition {
		val beginDateTime = if (begin != null && isOutOfPostgresRange(begin)) DSL.field(if (begin.year > 0) "'infinity'::timestamptz" else "'-infinity'::timestamptz", ZonedDateTime::class.java)
		else DSL.`val`(begin)
		val endDateTime = if (end != null && isOutOfPostgresRange(end)) DSL.field(if (end.year > 0) "'infinity'::timestamptz" else "'-infinity'::timestamptz", ZonedDateTime::class.java)
		else DSL.`val`(end)
	...
	fun validDateTime(id: UUID, begin: ZonedDateTime?, end: ZonedDateTime?): Mono<ProjectRelationEntity> {
		val profileIds = dsl.select(TB_PROJECT_PROFILE.ID).from(TB_PROJECT_PROFILE)
			.where(TB_PROJECT_PROFILE.PROJECT_ID.eq(id).and(availabilityNonOverlapCondition(TB_PROJECT_PROFILE.START_ACCESS_DATE, TB_PROJECT_PROFILE.START_ACCESS_TIME, TB_PROJECT_PROFILE.END_ACCESS_DATE, TB_PROJECT_PROFILE.END_ACCESS_TIME, begin, end)))
		...
		val participantIds = dsl.select(TB_PARTICIPANT.ID).from(TB_PARTICIPANT)
			.where(TB_PARTICIPANT.PROJECT_ID.eq(id).and(availabilityNonOverlapCondition(TB_PARTICIPANT.START_AVAILABILITY_DATE, TB_PARTICIPANT.START_AVAILABILITY_TIME, TB_PARTICIPANT.END_AVAILABILITY_DATE, TB_PARTICIPANT.END_AVAILABILITY_TIME, begin, end)))
		val groupIds = dsl.select(TB_GROUP.ID).from(TB_GROUP)
			.where(TB_GROUP.PROJECT_ID.eq(id).and(availabilityNonOverlapCondition(TB_GROUP.START_AVAILABILITY_DATE, TB_GROUP.START_AVAILABILITY_TIME, TB_GROUP.END_AVAILABILITY_DATE, TB_GROUP.END_AVAILABILITY_TIME, begin, end)))
		val vehicleIds = dsl.select(TB_VEHICLE.ID).from(TB_VEHICLE)
			.where(TB_VEHICLE.PROJECT_ID.eq(id).and(availabilityNonOverlapCondition(TB_VEHICLE.START_AVAILABILITY_DATE, TB_VEHICLE.START_AVAILABILITY_TIME, TB_VEHICLE.END_AVAILABILITY_DATE, TB_VEHICLE.END_AVAILABILITY_TIME, begin, end)))
		val activityIds = dsl.select(TB_ACTIVITY.ID).from(TB_ACTIVITY)
			.where(TB_ACTIVITY.PROJECT_ID.eq(id).and(availabilityNonOverlapCondition(TB_ACTIVITY.START_AVAILABILITY_DATE, TB_ACTIVITY.START_AVAILABILITY_TIME, TB_ACTIVITY.END_AVAILABILITY_DATE, TB_ACTIVITY.END_AVAILABILITY_TIME, begin, end)))
		val communicationIds = dsl.select(TB_COMMUNICATION.ID).from(TB_COMMUNICATION)
			.where(TB_COMMUNICATION.PROJECT_ID.eq(id).and(dateTimeNonOverlapCondition(TB_COMMUNICATION.DATE_TIME, begin, end)))
		...
		val union = profileIds.union(movementIds).union(participantIds).union(groupIds).union(vehicleIds).union(activityIds).union(communicationIds).union(alertIds)
			.asTable("t")
	...
	fun findProjectsEligibleForPurge(dateThreshold: LocalDate): Flux<UUID> {
		fun lastChangeCte(cteName: String, projectIdField: org.jooq.TableField<*, UUID?>, lastModifiedField: Field<ZonedDateTime?>) =
			name(cteName).`as`(
		...
		val lastProfileChange = lastChangeCte("last_profile_change", TB_PROJECT_PROFILE.PROJECT_ID, TB_PROJECT_PROFILE.LAST_MODIFIED_DATE)
		val lastMovementChange = lastChangeCte("last_movement_change", TB_MOVEMENT.PROJECT_ID, TB_MOVEMENT.LAST_MODIFIED_DATE)
		val lastParticipantChange = lastChangeCte("last_participant_change", TB_PARTICIPANT.PROJECT_ID, TB_PARTICIPANT.LAST_MODIFIED_DATE)
		val lastGroupChange = lastChangeCte("last_group_change", TB_GROUP.PROJECT_ID, TB_GROUP.LAST_MODIFIED_DATE)
		val lastVehicleChange = lastChangeCte("last_vehicle_change", TB_VEHICLE.PROJECT_ID, TB_VEHICLE.LAST_MODIFIED_DATE)
		val lastActivityChange = lastChangeCte("last_activity_change", TB_ACTIVITY.PROJECT_ID, TB_ACTIVITY.LAST_MODIFIED_DATE)
		val lastCommunicationChange = lastChangeCte("last_communication_change", TB_COMMUNICATION.PROJECT_ID, TB_COMMUNICATION.LAST_MODIFIED_DATE)
		...
		return Flux.from(
			dsl.with(lastProfileChange, lastMovementChange, lastParticipantChange, lastGroupChange, lastVehicleChange, lastActivityChange, lastCommunicationChange, lastAlertChange)
	...
	private fun insert(entity: ProjectEntity): Mono<ProjectEntity> {
		...
		entity.visible?.let { record.set(TB_PROJECT.VISIBLE, it) }
		entity.createdAt?.let { record.set(TB_PROJECT.CREATED_DATE, it) }
		entity.creatorId?.let { record.set(TB_PROJECT.CREATED_BY, it) }
		entity.lastUpdateAt?.let { record.set(TB_PROJECT.LAST_MODIFIED_DATE, it) }
		entity.lastEditorId?.let { record.set(TB_PROJECT.LAST_MODIFIED_BY, it) }
	...
	private fun update(entity: ProjectEntity): Mono<ProjectEntity> = Mono.from(
		dsl.update(TB_PROJECT)
			.set(TB_PROJECT.VISIBLE, entity.visible)
			.set(TB_PROJECT.CREATED_DATE, entity.createdAt)
			.set(TB_PROJECT.CREATED_BY, entity.creatorId)
			.set(TB_PROJECT.LAST_MODIFIED_DATE, entity.lastUpdateAt)
			.set(TB_PROJECT.LAST_MODIFIED_BY, entity.lastEditorId)
	...
	private fun Record.toEntity(creator: TbUser? = null, editor: TbUser? = null, fullCount: Field<Int>? = null): ProjectEntity = ProjectEntity(
		...
		id = get(TB_PROJECT.ID)
		visible = get(TB_PROJECT.VISIBLE)
		createdAt = get(TB_PROJECT.CREATED_DATE)
		creatorId = get(TB_PROJECT.CREATED_BY)
		...
		lastUpdateAt = get(TB_PROJECT.LAST_MODIFIED_DATE)
		lastEditorId = get(TB_PROJECT.LAST_MODIFIED_BY)
