...
interface IGroupContentEntityRepository: ReactiveCrudRepository<GroupContentEntity, UUID> {
    @Query(
        ...
        WHERE gt.$LINKED_PROJECT_ID = :projectId AND t.$GROUP_CONTENT_GROUP_ID IN (:groupIds)
    """
    )
    fun findAllByGroupIds(projectId: UUID, groupIds: List<UUID>): Flux<GroupContentEntity>
