...
interface IProjectV2Controller {
	@Operation(
		...
		description = "Paginated Projects visible to the caller",
	)
	...
	@Operation(
		summary = "Find the caller's projects with open alerts",
		description = "Projects the caller can access that have open (IN_PROGRESS) alerts, most-recent first, with the open-alert count (home dashboard)",
	)
	@PreAuthorize("isAuthenticated()")
	@GetMapping("/open-alerts")
	fun findOpenAlertProjects(
		@AuthenticationPrincipal currentUser: CurrentUserModel,
		@RequestParam(defaultValue = "5") @Valid @Min(1, message = PAGE_SIZE_IS_LOWER_THAN_ONE) @Max(
			200,
			message = PAGE_SIZE_IS_UPPER_THAN_MAX_PAGE_SIZE
		) limit: Int,
