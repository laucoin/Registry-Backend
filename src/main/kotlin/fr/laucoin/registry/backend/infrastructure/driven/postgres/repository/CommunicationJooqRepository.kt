...
class CommunicationJooqRepository(private val dsl: DSLContext) {
	...
	private fun insert(entity: CommunicationEntity): Mono<CommunicationEntity> {
		...
		entity.visible?.let { record.set(TB_COMMUNICATION.VISIBLE, it) }
		entity.createdAt?.let { record.set(TB_COMMUNICATION.CREATED_DATE, it) }
		entity.creatorId?.let { record.set(TB_COMMUNICATION.CREATED_BY, it) }
		entity.lastUpdateAt?.let { record.set(TB_COMMUNICATION.LAST_MODIFIED_DATE, it) }
		entity.lastEditorId?.let { record.set(TB_COMMUNICATION.LAST_MODIFIED_BY, it) }
		entity.projectId?.let { record.set(TB_COMMUNICATION.PROJECT_ID, it) }
	...
	private fun update(entity: CommunicationEntity): Mono<CommunicationEntity> = Mono.from(
		dsl.update(TB_COMMUNICATION)
			.set(TB_COMMUNICATION.VISIBLE, entity.visible)
			.set(TB_COMMUNICATION.CREATED_DATE, entity.createdAt)
			.set(TB_COMMUNICATION.CREATED_BY, entity.creatorId)
			.set(TB_COMMUNICATION.LAST_MODIFIED_DATE, entity.lastUpdateAt)
			.set(TB_COMMUNICATION.LAST_MODIFIED_BY, entity.lastEditorId)
			.set(TB_COMMUNICATION.PROJECT_ID, entity.projectId)
	...
	private fun Record.toEntity(
		...
		id = get(TB_COMMUNICATION.ID)
		visible = get(TB_COMMUNICATION.VISIBLE)
		createdAt = get(TB_COMMUNICATION.CREATED_DATE)
		creatorId = get(TB_COMMUNICATION.CREATED_BY)
		...
		lastUpdateAt = get(TB_COMMUNICATION.LAST_MODIFIED_DATE)
		lastEditorId = get(TB_COMMUNICATION.LAST_MODIFIED_BY)
		...
		projectId = get(TB_COMMUNICATION.PROJECT_ID)
