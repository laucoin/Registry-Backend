...
class MovementJooqRepository(private val dsl: DSLContext) {
	...
	// Mirrors MovementQueries.LAST_PARTICIPANT_MOVEMENT_JOIN's inner subquery: per participant, the MAX
	// visible movement date_time. The outer join only matches on date_time equality (not participant_id),
	// exactly like the original raw SQL — replicated as-is.
	private fun lastMovementPerParticipant() = dsl.select(max(TB_MOVEMENT.DATE_TIME).`as`("participant_last_movement_date_time"), TB_MOVEMENT_CONTENT.PARTICIPANT_ID)
		.from(TB_MOVEMENT)
	...
	// Mirrors MovementQueries.WITH_CURRENT_MOVEMENT's "last_participant_movement" CTE: re-exposes the above
	// per-participant last-movement as (movement id, movement type, participant id).
	...
	// Mirrors WITH_CURRENT_MOVEMENT's "filtered_groups" CTE, minus the unused "groups" JSON_AGG column: the
	// original SQL computes it via a LEFT JOIN to (unaliased) tb_group that is never referenced anywhere
	// downstream (current_movement only reads min/max_start/end_availability) — dropping that dead
	// computation changes no observable row, since neither join can filter rows away (both are LEFT JOINs).
	...
	// Builds the 3 chained CTEs behind WITH_CURRENT_MOVEMENT / CURRENT_MOVEMENT_JOIN: current_movement
	// resolves, per participant, the single movement that represents their current in/out state.
	...
	private fun dateRangeOverlap(startDateTimeSearched: ZonedDateTime?, endDateTimeSearched: ZonedDateTime?): Condition {
	...
	private fun baseSelect(activity: TbActivity, project: TbProject, creator: TbUser, editor: TbUser, vararg extraFields: SelectField<*>): SelectOnConditionStep<Record> =
		dsl.select(
			listOf(
				TB_MOVEMENT.asterisk(),
				activity.ID, activity.NAME, activity.DESCRIPTION, activity.DURATION, activity.MIN_ALLOWED_PARTICIPANTS, activity.MAX_ALLOWED_PARTICIPANTS,
				activity.START_AVAILABILITY_DATE, activity.START_AVAILABILITY_TIME, activity.END_AVAILABILITY_DATE, activity.END_AVAILABILITY_TIME,
	...
	fun findAll(
		...
		return Flux.from(
			baseSelect(activity, project, creator, editor, fullCount)
				.where(searchConditions(projectId, visibilitySearched, linkedToActivity, typeSearched, startDateTimeSearched, endDateTimeSearched))
	...
	fun findAllByParticipantId(
		...
		return Flux.from(
			baseSelect(activity, project, creator, editor, fullCount)
				.join(TB_MOVEMENT_CONTENT).on(TB_MOVEMENT_CONTENT.MOVEMENT_ID.eq(TB_MOVEMENT.ID).and(TB_MOVEMENT_CONTENT.PARTICIPANT_ID.eq(participantId)))
				.where(searchConditions(projectId, visibilitySearched, linkedToActivity, typeSearched, startDateTimeSearched, endDateTimeSearched))
				.orderBy(TB_MOVEMENT.DATE_TIME.desc())
				.limit(limit).offset(offset)
		).map { it.toEntity(activity, project, creator, editor, fullCount) }
	...
	fun countAllByParticipantId(
		...
		dsl.select(count(TB_MOVEMENT.ID))
			...
			.where(TB_MOVEMENT_CONTENT.PARTICIPANT_ID.eq(participantId).and(searchConditions(projectId, visibilitySearched, linkedToActivity, typeSearched, startDateTimeSearched, endDateTimeSearched)))
	).map { it.value1()!!.toLong() }
	...
	fun findAllByVehicleId(
		...
		return Flux.from(
			baseSelect(activity, project, creator, editor, fullCount)
				.join(TB_MOVEMENT_CONTENT).on(TB_MOVEMENT_CONTENT.MOVEMENT_ID.eq(TB_MOVEMENT.ID).and(TB_MOVEMENT_CONTENT.VEHICLE_ID.eq(vehicleId)))
				.where(searchConditions(projectId, visibilitySearched, linkedToActivity, typeSearched, startDateTimeSearched, endDateTimeSearched))
				.orderBy(TB_MOVEMENT.DATE_TIME.desc())
				.limit(limit).offset(offset)
		).map { it.toEntity(activity, project, creator, editor, fullCount) }
	...
	fun countAllByVehicleId(
		...
		dsl.select(count(TB_MOVEMENT.ID))
			.from(TB_MOVEMENT_CONTENT)
			.join(TB_MOVEMENT).on(TB_MOVEMENT_CONTENT.MOVEMENT_ID.eq(TB_MOVEMENT.ID))
			.where(TB_MOVEMENT_CONTENT.VEHICLE_ID.eq(vehicleId).and(searchConditions(projectId, visibilitySearched, linkedToActivity, typeSearched, startDateTimeSearched, endDateTimeSearched)))
	).map { it.value1()!!.toLong() }
	...
	fun findByActivityWithLimit(
		...
		val plm = lastMovementPerParticipant()
		val plmDateTime = field(name("plm", "participant_last_movement_date_time"), ZonedDateTime::class.java)
		val similarityScore = (if (textSearched == null) DSL.inline(1f) else similarity(activity.SEARCH_TEXT, DSL.`val`(textSearched))).`as`("similarity_score")

		val conditions = mutableListOf(
			TB_MOVEMENT.PROJECT_ID.eq(projectId),
			TB_MOVEMENT.ACTIVITY_ID.isNotNull.and(textSearched?.let { similarity(activity.SEARCH_TEXT, DSL.`val`(it)).gt(0f) } ?: DSL.noCondition()),
			visibleCondition(TB_MOVEMENT.VISIBLE, visibilitySearched),
		)
		availabilitySearched?.let {
			val cond = currentlyUsableCondition(activity.START_AVAILABILITY_DATE, activity.START_AVAILABILITY_TIME, activity.END_AVAILABILITY_DATE, activity.END_AVAILABILITY_TIME)
			conditions += if (it) cond else cond.not()
		}
		dateTimeSearched?.let { conditions += dateRangeCondition(activity.START_AVAILABILITY_DATE, activity.START_AVAILABILITY_TIME, activity.END_AVAILABILITY_DATE, activity.END_AVAILABILITY_TIME, it) }

		return Flux.from(
			dsl.select(
				listOf(
					TB_MOVEMENT.asterisk(), similarityScore,
					activity.ID, activity.NAME, activity.DESCRIPTION, activity.DURATION, activity.MIN_ALLOWED_PARTICIPANTS, activity.MAX_ALLOWED_PARTICIPANTS,
					activity.START_AVAILABILITY_DATE, activity.START_AVAILABILITY_TIME, activity.END_AVAILABILITY_DATE, activity.END_AVAILABILITY_TIME,
					project.NAME, project.BEGIN_DATE, project.BEGIN_TIME, project.END_DATE, project.END_TIME, project.OPTIONS,
					creator.FIRST_NAME, creator.LAST_NAME, creator.EMAIL, editor.FIRST_NAME, editor.LAST_NAME, editor.EMAIL,
				...
				.groupBy(
					TB_MOVEMENT.ID, TB_MOVEMENT.DATE_TIME, TB_MOVEMENT.TYPE, TB_MOVEMENT.ACTIVITY_ID, TB_MOVEMENT.REASON, activity.SEARCH_TEXT,
					project.ID, project.NAME, project.BEGIN_DATE, project.BEGIN_TIME, project.END_DATE, project.END_TIME, project.OPTIONS,
					activity.ID, activity.NAME, activity.DESCRIPTION, activity.DURATION, activity.MIN_ALLOWED_PARTICIPANTS, activity.MAX_ALLOWED_PARTICIPANTS,
					activity.START_AVAILABILITY_DATE, activity.START_AVAILABILITY_TIME, activity.END_AVAILABILITY_DATE, activity.END_AVAILABILITY_TIME,
					creator.FIRST_NAME, creator.LAST_NAME, creator.EMAIL, editor.FIRST_NAME, editor.LAST_NAME, editor.EMAIL,
					TB_MOVEMENT.VISIBLE, TB_MOVEMENT.CREATED_BY, TB_MOVEMENT.CREATED_DATE, TB_MOVEMENT.LAST_MODIFIED_BY, TB_MOVEMENT.LAST_MODIFIED_DATE,
				)
	...
	fun findById(projectId: UUID, id: UUID, visibilitySearched: Boolean?): Mono<MovementEntity> {
		...
		return Mono.from(
			baseSelect(activity, project, creator, editor)
				.where(TB_MOVEMENT.PROJECT_ID.eq(projectId).and(TB_MOVEMENT.ID.eq(id)).and(visibleCondition(TB_MOVEMENT.VISIBLE, visibilitySearched)))
		).map { it.toEntity(activity, project, creator, editor) }
	}
	...
	fun save(entity: MovementEntity, using: DSLContext): Mono<MovementEntity> = if (entity.id == null) insert(using, entity) else update(using, entity)

	private fun insert(using: DSLContext, entity: MovementEntity): Mono<MovementEntity> {
		val record = using.newRecord(TB_MOVEMENT)
		entity.visible?.let { record.set(TB_MOVEMENT.VISIBLE, it) }
		entity.createdAt?.let { record.set(TB_MOVEMENT.CREATED_DATE, it) }
		entity.creatorId?.let { record.set(TB_MOVEMENT.CREATED_BY, it) }
		entity.lastUpdateAt?.let { record.set(TB_MOVEMENT.LAST_MODIFIED_DATE, it) }
		entity.lastEditorId?.let { record.set(TB_MOVEMENT.LAST_MODIFIED_BY, it) }
		entity.projectId?.let { record.set(TB_MOVEMENT.PROJECT_ID, it) }
	...
	private fun update(using: DSLContext, entity: MovementEntity): Mono<MovementEntity> = Mono.from(
		using.update(TB_MOVEMENT)
			.set(TB_MOVEMENT.VISIBLE, entity.visible)
			.set(TB_MOVEMENT.CREATED_DATE, entity.createdAt)
			.set(TB_MOVEMENT.CREATED_BY, entity.creatorId)
			.set(TB_MOVEMENT.LAST_MODIFIED_DATE, entity.lastUpdateAt)
			.set(TB_MOVEMENT.LAST_MODIFIED_BY, entity.lastEditorId)
			.set(TB_MOVEMENT.PROJECT_ID, entity.projectId)
	...
	private fun Record.toEntity(
		...
		id = get(TB_MOVEMENT.ID)
		visible = get(TB_MOVEMENT.VISIBLE)
		createdAt = get(TB_MOVEMENT.CREATED_DATE)
		creatorId = get(TB_MOVEMENT.CREATED_BY)
		...
		lastUpdateAt = get(TB_MOVEMENT.LAST_MODIFIED_DATE)
		lastEditorId = get(TB_MOVEMENT.LAST_MODIFIED_BY)
		...
		projectId = get(TB_MOVEMENT.PROJECT_ID)
