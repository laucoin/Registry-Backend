...
class MovementContentJooqRepository(private val dsl: DSLContext) {
	// Duplicated from MovementJooqRepository (small, matches this migration's established style of not
	// sharing CTE-builder helpers across repository classes) — see that class for the detailed comments on
	// the WITH_CURRENT_MOVEMENT port.
	...
	fun findAllByMovementIds(projectId: UUID, movementIds: List<UUID>): Flux<MovementContentEntity> {
		...
		return Flux.from(
			dsl.select(
				...
				.join(TB_PARTICIPANT).on(TB_MOVEMENT_CONTENT.PARTICIPANT_ID.eq(TB_PARTICIPANT.ID).and(TB_PARTICIPANT.PURGED.isFalse))
	...
	fun findCurrentByMovementIds(projectId: UUID, movementIds: List<UUID>): Flux<MovementContentEntity> {
		...
		return Flux.from(
			dsl.with(lastParticipantMovement, filteredGroups, currentMovement)
				...
				.join(TB_MOVEMENT_CONTENT).on(TB_MOVEMENT_CONTENT.MOVEMENT_ID.eq(cmId).and(TB_MOVEMENT_CONTENT.PARTICIPANT_ID.eq(cmParticipantId)))
				.join(TB_MOVEMENT).on(TB_MOVEMENT_CONTENT.MOVEMENT_ID.eq(TB_MOVEMENT.ID))
				.join(TB_PARTICIPANT).on(TB_MOVEMENT_CONTENT.PARTICIPANT_ID.eq(TB_PARTICIPANT.ID).and(TB_PARTICIPANT.PURGED.isFalse))
				.leftJoin(TB_VEHICLE).on(TB_MOVEMENT_CONTENT.VEHICLE_ID.eq(TB_VEHICLE.ID))
	...
	private fun Record.toEntity(includeParticipantAndVehicle: Boolean = false): MovementContentEntity = MovementContentEntity(
