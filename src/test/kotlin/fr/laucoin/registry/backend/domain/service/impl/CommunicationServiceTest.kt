...
class CommunicationServiceTest {
	...
	private val service: ICommunicationService =
		CommunicationService(projectService, port, movementPort, alertPort, 1, 1)
	...
	fun `Should createCommunication check date and call port create`() {
		...
		whenever(movementPort.findById(any(), anyOrNull(), anyOrNull())).thenReturn(Mono.just(movement))
		...
		verify(movementPort).findById(projectId, movementId, visibilitySearched = null)
	...
	fun `Should createCommunication throw because of not found movement`() {
		...
		val communication =
			CommunicationModel(dateTime = now, movement = MovementModel().apply { id = movementUuid }).apply {
				project = ProjectModel().apply { id = projectId }
			}
		whenever(projectService.validateDateTime(any(), anyOrNull(), any())).thenReturn(Mono.just(projectId))
		whenever(movementPort.findById(any(), anyOrNull(), anyOrNull())).thenReturn(Mono.empty())
		whenever(port.create(any())).thenReturn(Mono.just(communication))
		...
		verify(movementPort).findById(projectId, movementUuid, visibilitySearched = null)
	...
	fun `Should createCommunication throw because of movement is not visible`() {
		...
		val now = ZonedDateTime.now()
		val communication =
			CommunicationModel(dateTime = now, movement = MovementModel().apply { id = movementId }).apply {
				project = ProjectModel().apply { id = projectId }
			}
		whenever(projectService.validateDateTime(any(), anyOrNull(), any())).thenReturn(Mono.just(projectId))
		whenever(movementPort.findById(any(), any(), anyOrNull())).thenReturn(Mono.just(MovementModel().apply {
			dateTime = now
			visible = false
		}))
		whenever(port.create(any())).thenReturn(Mono.just(communication))
	...
	fun `Should updateCommunicationById check date, check existing communication, call port updateCommunication`() {
		...
		whenever(port.findById(any(), any(), anyOrNull())).thenReturn(Mono.just(communication))
		whenever(
			movementPort.findById(
				any(),
				anyOrNull(),
				anyOrNull()
			)
		).thenReturn(Mono.just(MovementModel().apply {
			visible = true
		}))
		...
		verify(movementPort, never()).findById(projectId, movementUuid, visibilitySearched = null)
	...
	fun `Should disableCommunicationById call existing communication and call port update`() {
		// Arrange
		val communication =
			CommunicationModel().apply { project = ProjectModel().apply { id = projectId }; visible = true }
		val uuid = UUID.randomUUID()
		whenever(port.findById(any(), any(), anyOrNull())).thenReturn(Mono.just(communication))
		whenever(port.update(any())).thenReturn(Mono.just(communication))
		...
		verify(port).update(communication.apply { visible = false })
	}
	...
	fun `Should enableCommunicationById call existing communication and call port update`() {
		// Arrange
		val communication =
			CommunicationModel().apply { project = ProjectModel().apply { id = projectId }; visible = false }
		val uuid = UUID.randomUUID()
		whenever(port.findById(any(), any(), anyOrNull())).thenReturn(Mono.just(communication))
		whenever(port.update(any())).thenReturn(Mono.just(communication))
		...
		verify(port).update(communication.apply { visible = true })
	}
