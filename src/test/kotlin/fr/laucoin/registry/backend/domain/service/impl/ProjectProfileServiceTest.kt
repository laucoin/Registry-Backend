...
class ProjectProfileServiceTest {
    ...
    private val service = ProjectProfileService(profileService, repository, roleService, userRepository, maxUser)

    companion object {
        @JvmStatic
        fun `Should createProjectProfiles call repository findUserIdsWithProjectProfile and saveAll`(): Stream<Arguments> {
        ...
        @JvmStatic
        fun `Should updateProjectProfileById call repository findById, findUserIdsWithProjectProfile and throw because user are not allowed to edit that role`(): Stream<Arguments> {
            return Stream.of(
                Arguments.of(
    ...
    @Test
    fun `Should findProjectProfilesPage call repository findProjectProfilesPageByProjectId`() {
        // Arrange
        val pageable = PageableModel(0, 10)
        val params = ProjectProfileSearchParamModel(statusSearched = ACCEPTED)
        whenever(repository.findProjectProfilesPageByProjectId(any(), any(), any())).thenReturn(
        ...
        // Assert
        verify(repository).findProjectProfilesPageByProjectId(projectId, pageable, params)
    }

    @Test
    fun `Should findProjectProfileById call repository findById`() {
        // Arrange
    ...
    @Test
    fun `Should findProjectProfileById call repository findById throw on empty result`() {
        // Arrange
        val uuid = UUID.randomUUID()
        val onlyVisible = true
    ...
    @Test
    fun `Should getAssignableProjectRoles call repository findProjectProfileByProjectAndUserId and role service getAssignableProjectRoles`() {
        // Arrange
        val profile = ProjectProfileModel().apply {
            role = "PROJECT_ROLE"; project = ProjectModel().apply { id = projectId }
        }
        whenever(repository.findProjectProfileByProjectAndUserId(any(), any(), anyOrNull())).thenReturn(Mono.just(profile))
        ...
        // Assert
        verify(repository).findProjectProfileByProjectAndUserId(
            projectId,
            currentUser().id !!,
    ...
    @Test
    fun `Should getAssignableProjectRoles throw on repository findProjectProfileByProjectAndUserId return empty`() {
        // Arrange
        whenever(repository.findProjectProfileByProjectAndUserId(any(), any(), anyOrNull())).thenReturn(Mono.empty())
        whenever(roleService.getAssignableProjectRoles(any())).thenReturn(emptyList())
    ...
    @ParameterizedTest
    @MethodSource
    fun `Should createProjectProfiles call repository findUserIdsWithProjectProfile and saveAll`(
        wantedProfileForUserIds: List<UUID>,
        userIdWithExistingProfile: List<UUID>,
        expectedCreatedUserIds: List<UUID>,
    ) {
        // Arrange
        val profiles = wantedProfileForUserIds.map { ProjectProfileModel().apply { user = UserModel().apply { id = it } } }
    ...
    @Test
    fun `Should createProjectProfiles call repository findUserIdsWithProjectProfile and throw because all profiles duplicated`() {
        // Arrange
        val uuid = UUID.randomUUID()
    ...
    @Test
    fun `Should updateProjectProfileById call repository findById, findUserIdsWithProjectProfile, call profileService validateNotLastProjectRoleLevel0, call repository findProjectProfileByProjectAndUserId, call roleService getAssignableProjectRoles and finally call repository update`() {
        // Arrange
        val uuid = UUID.randomUUID()
        ...
        whenever(profileService.validateNotLastProjectRoleLevel0(any(), any(), any(), any())).thenReturn(Mono.just(profile))
        whenever(repository.findProjectProfileByProjectAndUserId(any(), any(), any())).thenReturn(Mono.just(currentUserProfile))
    ...
    @Test
    fun `Should updateProjectProfileById call repository findById, findUserIdsWithProjectProfile, call profileService validateNotLastProjectRoleLevel0, call repository findProjectProfileByProjectAndUserId, call roleService getAssignableProjectRoles and throw because profile duplicated`() {
        // Arrange
        val uuid = UUID.randomUUID()
        val profileRole = "ROLE_PROJECT"
        val profile = ProjectProfileModel().apply {
        ...
        verify(repository, never()).findProjectProfileByProjectAndUserId(any(), any(), any())
        verify(roleService, never()).getAssignableProjectRoles(any())
    ...
    @ParameterizedTest
    @MethodSource
    fun `Should updateProjectProfileById call repository findById, findUserIdsWithProjectProfile and throw because user are not allowed to edit that role`(
    ...
    @Test
    fun `Should blockProjectProfileById call repository findById, call service profile validateNotLastProjectRoleLevel0 and finally call repository update`() {
        // Arrange
        val uuid = UUID.randomUUID()
        val profile = ProjectProfileModel().apply {
            user = UserModel().apply { id = uuid }
    ...
    @Test
    fun `Should unblockProjectProfileById call repository findById, call service profile validateNotLastProjectRoleLevel0 and finally call repository update`() {
        // Arrange
        val uuid = UUID.randomUUID()
        val profile = ProjectProfileModel().apply {
            user = UserModel().apply { id = uuid }
    ...
    @Test
    fun `Should deleteProjectProfileById call repository findById, call service profile validateNotLastProjectRoleLevel0 and finally call repository deleteById`() {
        // Arrange
        val uuid = UUID.randomUUID()
        val profile = ProjectProfileModel().apply {
            user = UserModel().apply { id = uuid }
