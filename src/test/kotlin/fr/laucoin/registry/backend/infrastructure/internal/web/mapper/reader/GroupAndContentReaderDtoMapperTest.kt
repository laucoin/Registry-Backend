...
class GroupAndContentReaderDtoMapperTest {
    private val participantMapper: ParticipantReaderDtoMapper = mock()
    private val mapper: GroupAndContentReaderDtoMapper = GroupAndContentReaderDtoMapper(participantMapper)
    ...
    fun `Should toDto convert GroupModel to GroupAndContentReaderDto`() {
        ...
        // Act
        val result = mapper.toDto(group)

        // Assert
        verify(participantMapper, times(1)).toDtoList(group.members)

        assertEquals(group.id, result.id)
        assertEquals(0, result.members.size)
        assertEquals(group.event, result.event)
