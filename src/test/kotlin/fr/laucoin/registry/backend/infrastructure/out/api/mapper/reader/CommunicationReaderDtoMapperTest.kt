...
class CommunicationReaderDtoMapperTest {
	...
	private val mapper: CommunicationReaderDtoMapper =
		CommunicationReaderDtoMapper(projectMapper, movementMapper, alertMapper)
	...
	fun `Should toDto convert CommunicationModel to CommunicationReaderDto`(
		...
		verify(projectMapper, times(expectedProjectCast)).toDto(
			communication.project ?: ProjectModel(),
			Locale.getDefault()
		)
		verify(movementMapper, times(expectedMovementCast)).toDto(
			communication.movement ?: MovementModel(),
			Locale.getDefault()
		)
