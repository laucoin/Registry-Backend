...
- **Comment policy (AGENTS.md § Comment policy)**: flag inline explanations or narration inside method bodies,
  end-of-line (trailing) comments, commented-out code, and signature-restating docstrings. Intent documentation belongs
  in one English KDoc block comment (`/** … */`) directly above a complex/non-obvious declaration — flag a `//` block
  used for documentation. In tests, `// Arrange` / `// Act` / `// Assert` structure comments are mandatory — flag their
  absence, and flag any change that deletes or edits them.
