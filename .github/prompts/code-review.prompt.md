...
- `infrastructure/driving` doesn't depend on `infrastructure/driven`; nothing depends on `config`; postgres `entity`
  stays inside its package; controllers implement a contract interface.
...
- Stays stateless (no session/form-login regressions); CORS not widened to `*`; security headers stay in
  `SecurityConfig`'s `.headers{}` block.
